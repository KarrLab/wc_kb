...
class KbTransformer:
    ...
    def __init__(self, kb_path=None, column=None):
    ...
    def transformSpeciesProperties(self):
        """ Parses information from the KB's 'Species Properties'. """

        #kbSTP.cell(column=self.header['speciestype properties']['id'], row=max_row+1).value =
        #kbSTP.cell(column=self.header['speciestype properties']['species type'], row=max_row+1).value =
        #kbSTP.cell(column=self.header['speciestype properties']['property'], row=max_row+1).value =
        #kbSTP.cell(column=self.header['speciestype properties']['value'], row=max_row+1).value =
        #kbSTP.cell(column=self.header['speciestype properties']['units'], row=max_row+1).value =
        #kbSTP.cell(column=self.header['speciestype properties']['evidence'], row=max_row+1).value =
        #kbSTP.cell(column=self.header['speciestype properties']['database reference'], row=max_row+1).value =
        #kbSTP.cell(column=self.header['speciestype properties']['references'], row=max_row+1).value =
        #kbSTP.cell(column=self.header['speciestype properties']['comments'], row=max_row+1).value =

        # Copy properties from old species properties
        """
        kbOld = self.kb['Species properties']
        kbSTP = self.kb['SpeciesType properties']
        for rowIdx in range(2, kbOld.max_row):
            print(rowIdx)
            speciesTypeId = kbOld.cell(column=self.header['species properties']['id'], row=rowIdx).value[6:]

            # Check if strucutre exists
            if kbOld.cell(column=self.header['species properties']['structure'], row=rowIdx).value is not None:
                row = kbSTP.max_row+1

                kbSTP.cell(column=self.header['speciestype properties']['id'], row=row).value = 'PROP({}:{})'.format(speciesTypeId, 'structure')
                kbSTP.cell(column=self.header['speciestype properties']['species type'], row=row).value = speciesTypeId
                kbSTP.cell(column=self.header['speciestype properties']['property'], row=row).value = 'structure'
                kbSTP.cell(column=self.header['speciestype properties']['units'], row=row).value = 'dimensionless'
                kbSTP.cell(column=self.header['speciestype properties']['value'], row=row).value = \
                    kbOld.cell(column=self.header['species properties']['structure'], row=rowIdx).value


            # Check if half life
            if kbOld.cell(column=self.header['species properties']['half life'], row=rowIdx).value is not None:
                row = kbSTP.max_row+1

                kbSTP.cell(column=self.header['speciestype properties']['id'], row=row).value = 'PROP({}:{})'.format(speciesTypeId, 'half_life')
                kbSTP.cell(column=self.header['speciestype properties']['species type'], row=row).value = speciesTypeId
                kbSTP.cell(column=self.header['speciestype properties']['property'], row=row).value = 'half_life'
                kbSTP.cell(column=self.header['speciestype properties']['units'], row=row).value = 'min'
                kbSTP.cell(column=self.header['speciestype properties']['value'], row=row).value = \
                    kbOld.cell(column=self.header['species properties']['half life'], row=rowIdx).value
                kbSTP.cell(column=self.header['speciestype properties']['evidence'], row=row).value = \
                    kbOld.cell(column=self.header['species properties']['evidence'], row=rowIdx).value
        """

        # Copy properties from 'Proteins' sheet
        ...
        properties = ['localization',
                      ...
                      'dna footprint binding',
                      'dna footprint length']

        for rowIdx in range(2, kbProteins.max_row):
            print('Protein row: {}'.format(rowIdx))
            speciesTypeId = kbProteins.cell(column=self.header['proteins']['id'], row=rowIdx).value

            for property in properties:
                if kbProteins.cell(column=self.header['proteins'][property], row=rowIdx).value is not None:
                    row = kbSTP.max_row+1

                    kbSTP.cell(column=self.header['speciestype properties']['id'], row=row).value = 'PROP({}:{})'.format(speciesTypeId, property)
                    kbSTP.cell(column=self.header['speciestype properties']['species type'], row=row).value = speciesTypeId
                    kbSTP.cell(column=self.header['speciestype properties']['property'], row=row).value = property
                    kbSTP.cell(column=self.header['speciestype properties']['value'], row=row).value = \
                        str(kbProteins.cell(column=self.header['proteins'][property], row=rowIdx).value)

                    if property == 'translation rate':
                        kbSTP.cell(column=self.header['speciestype properties']['units'], row=row).value = \
                            kbProteins.cell(column=self.header['proteins']['unit'], row=rowIdx).value
                    elif 'length' in property:
                        kbSTP.cell(column=self.header['speciestype properties']['units'], row=row).value = 'nt'
                    else:
                        kbSTP.cell(column=self.header['speciestype properties']['units'], row=row).value = 'dimensionless'

                    self.concatenateId(kbProteins.cell(column=self.header['proteins']['species properties'], row=rowIdx),
                        kbSTP.cell(column=self.header['speciestype properties']['id'], row=row).value)

        # Copy properties from 'Complexes' sheet
        kbComplexes = self.kb['Complexes']
        kbSTP       = self.kb['SpeciesType properties']
        ...
        for rowIdx in range(2, kbComplexes.max_row):
            print('Complex row: {}'.format(rowIdx))
            speciesTypeId = kbComplexes.cell(column=self.header['complexes']['id'], row=rowIdx).value

            for property in properties:

                if kbComplexes.cell(column=self.header['complexes'][property], row=rowIdx).value is not None:
                    row = kbSTP.max_row+1
                    ...
                    kbSTP.cell(column=self.header['speciestype properties']['property'], row=row).value = property
                    kbSTP.cell(column=self.header['speciestype properties']['value'], row=row).value = \
                        str(kbComplexes.cell(column=self.header['complexes'][property], row=rowIdx).value)

                    if property == 'dna footprint length':
                        kbSTP.cell(column=self.header['speciestype properties']['units'], row=row).value = 'nt'
                    else:
                        kbSTP.cell(column=self.header['speciestype properties']['units'], row=row).value = 'dimensionless'

                    self.concatenateId(kbComplexes.cell(column=self.header['complexes']['species properties'], row=rowIdx),
                        kbSTP.cell(column=self.header['speciestype properties']['id'], row=row).value )
...
class KbTranslater:
    ...
    def translateChromosomeFeatures(self):
        ...
        for rowIdx in range(3, coreChromFeats.max_row+1):
            ...
            start = kbChromFeats.cell(column=self.header['Chromosome features']['Coordinate'], row=rowIdx-1).value
            length = kbChromFeats.cell(column=self.header['Chromosome features']['Length'], row=rowIdx-1).value
            dir = kbChromFeats.cell(column=self.header['Chromosome features']['Direction'], row=rowIdx-1).value

            if not ((start is not None) and (length is not None) and (dir is not None)):
                continue
            ...
            kbChromFeats.cell(column=self.header['Chromosome features']['End'], row=rowIdx-1).value = end

            txt = coreChromFeats.cell(column=12, row=rowIdx).value
            if txt is None:
                continue

            chromFeatDict = json.loads(txt)
            evidenceEntry = self.createEvidence(
                objectId = chromeFeatId,
                property = 'intensity',
                values = float(chromFeatDict['value']),
    ...
    def convertRnaCoordinates(self):
        ...
        for rowIdx in range(3, coreRnas.max_row+1):
            # Check if row is correct
            print(rowIdx)
            assert kbRnas.cell(column=1, row=rowIdx-1).value == coreRnas.cell(column=1, row=rowIdx).value

            start = kbRnas.cell(column=self.header['RNAs']['Coordinate'], row=rowIdx-1).value
            length = kbRnas.cell(column=self.header['RNAs']['Length'], row=rowIdx-1).value
            dir = kbRnas.cell(column=self.header['RNAs']['Direction'], row=rowIdx-1).value
            ...
            kbRnas.cell(column=self.header['RNAs']['End'], row=rowIdx-1).value = end
